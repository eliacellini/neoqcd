#!/usr/bin/env python3
"""
Collect O-REX production-array logs into one combined txt and one CSV.

Run from the project root, for example:

    python main/collect_orex_array_results.py
"""
...
KEEP_PATTERNS = [
    re.compile(r"^Running O-REX production array task:"),
    re.compile(r"^SLURM_ARRAY_TASK_ID="),
    re.compile(r"^NREPLICAS="),
    re.compile(r"^OREX_PROTOCOL_STEPS="),
...
PATTERNS = {
    ...
    "nsteps": re.compile(r"\bOREX_PROTOCOL_STEPS=(?P<nsteps>\d+)"),
...
def sort_key(row: dict) -> tuple[int, int, int, str]:
    return (
        _as_int(row.get("nreplicas", row.get("world_size", -1))),
        _as_int(row.get("nsteps", -1)),
        _as_int(row.get("array_task_id", -1)),
        str(row.get("log_file", "")),
    )
...
def main() -> int:
    ...
    parser.add_argument("--out-prefix", type=str, default="orex_array_results")
    parser.add_argument(
        "--glob",
        action="append",
        default=[],
    ...
    if not paths:
        raise SystemExit(f"No O-REX array logs found under {reports_dir}")
