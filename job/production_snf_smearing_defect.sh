...
#SBATCH --job-name=neoqcd-snf-smearing-defect
...
BASE_RUN_NAME="${RUN_NAME:-production_snf_smearing_defect_beta6_bc0to1_D${D}_T${T}_L${L}_N${N}_d${DEFECT_SIZE}_t${TIME_SLICE}_s${SPACE_SLICE}_bs${BS}_K${PROTOCOL_STEPS}_h${HYPER_HIDDEN_DIM}_depth${HYPER_DEPTH}}"
...
run_phase() {
  ...
  CMD=(
    ...
    # Intentional: match the hyper-conditioned NEO-PT smearing job.
    --nf-architecture hyper-smearing
